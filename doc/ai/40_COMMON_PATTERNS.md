...
- Why it seems to exist: API names are known from config before specs are loaded; full operation commands require loading the selected spec.
...
- Why it seems to exist: keep parsing/config/neutral request contract/runtime behavior separately testable.
...
- Why it seems to exist: request building and response streaming can be tested or embedded without binding to `reqwest`/Tokio/Clap.
...
- Why it seems to exist: postcard cannot deserialize arbitrary `serde_json::Value` through `deserialize_any`, so cached IR uses `SpallValue`.
...
- Why it seems to exist: centralize `serde_saphyr` usage and enforce DoS budgets.
...
- Why it seems to exist: secrets must be held in redacting wrappers and not persisted accidentally.
...
- Why it seems to exist: OpenAPI can have parameters with the same name in different locations, while Clap arg IDs must be unique.
...
- Why it seems to exist: large paginated JSON responses must not be concatenated or buffered whole.
...
- Why it seems to exist: OpenAPI path/operation inheritance must be collapsed into Spall's resolved IR.
...
- Why it seems to exist: MCP stdio requires stdout to be JSON-RPC only.
